...
class convolve(object):
    ...
    def spaceConv(self):
        ...
        for i in trange(self.__rangeX_ - 2*offsetX):
    ...
    def OaAconvolve(self):
        ...
        kernel = np.pad(self.kernel, [(padY, padY), (padX, padX)], \
                    mode='constant', constant_values=0)

        plt.imshow(kernel, interpolation='none', cmap='gist_heat')
        plt.show()

        transformed_kernel = FFT(kernel)

        # create a blank array of the same size to lay them down
        convolved_image = np.zeros([self.array.shape[0], \
                                    self.array.shape[1]])
        ...
        for tup in tqdm(subsets):
            ...
            subset = np.pad(self.array[tup[0]:tup[1], tup[2]:tup[3]],      \
                            [(padY, padY), (padX, padX)], mode='constant', \
                            constant_values=0)
